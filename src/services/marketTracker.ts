...
import watcherPnLTracker from './watcherPnLTracker';
...
class MarketTracker {
    ...
    private is15MinOrHourlyMarket(activity: any): boolean {
        // Use combined text (slug + eventSlug + title) so we don't miss
        // 15m markets where the token slug doesn't contain "15m" but the
        // question/title has the full "8:00AM-8:15AM ET" window.
        const slug = activity?.slug || '';
        const eventSlug = activity?.eventSlug || '';
        const title = activity?.title || activity?.asset || '';
        const rawText = `${slug} ${eventSlug} ${title}`;
        ...
        // 1) Explicit "15 min"/"15min" or "updown 15"
        // 2) Time range pattern like "6:00AM-6:15AM" (colon-based windows)
        const hasExplicit15Min =
            /\b15\s*min|\b15min|updown.*?15|15.*?updown/i.test(rawText);
        const hasTimeRange =
            /\d{1,2}:\d{2}\s*(?:am|pm)?\s*[-–]\s*\d{1,2}:\d{2}\s*(?:am|pm)?/i.test(rawText);
        const has15Min = hasExplicit15Min || hasTimeRange;

        // Check for hourly timeframe (1h, 1 hour, hourly)
        const hasHourly = /\b1\s*h|\b1\s*hour|\bhourly/i.test(rawText);

        // Check for hourly markets by pattern: "Up or Down" with single time (e.g., "6AM ET")
        // but NO time range. 15m markets have a time range with colons.
        const hasUpDown = /(?:up|down).*?(?:up|down)|updown/i.test(textLower);
        const hasCrypto = /(?:bitcoin|ethereum|btc|eth)/i.test(rawText);
        const hasSingleTime =
            /\d{1,2}\s*(?:am|pm)\s*et/i.test(rawText) ||
            /\d{1,2}(?:am|pm)-et/i.test(rawText);
     ...
     * Also handles 1-hour markets with single time patterns like "1PM ET"
    ...
    private isTimeWindowMarketPassed(marketName: string): boolean {
        ...
        // First, try to extract 15-min time window (e.g., "10:30AM-10:45AM")
        ...
        // For 1-hour markets: Check for single time pattern like "1PM ET" or "2PM ET"
        // Pattern: "Bitcoin Up or Down - January 13, 1PM ET"
        const hourlyPattern = /(\d{1,2})\s*(AM|PM)\s*ET/i;
        const hourlyMatch = marketName.match(hourlyPattern);
        if (hourlyMatch) {
            try {
                let startHour = parseInt(hourlyMatch[1], 10);
                const isPM = /PM/i.test(hourlyMatch[2]);
                ...
                if (endHour < startHour) {
                    // Crossed midnight - if we're past midnight but before end, not passed
                    // If we're after start but before midnight, not passed
                    if (currentTotalMinutes >= endTotalMinutes && currentTotalMinutes < startHour * 60) {
                        return true;
                    }
                    return false;
                }
        ...
        // Not a time-window or hourly market, can't determine if passed
        return false;
    }
    ...
    private removeOlderUpDown15Markets(newMarketKey: string, newMarketActivity: any): void {
        ...
        for (const key of marketsToRemove) {
            ...
            if (marketToRemove) {
                ...
                if (!this.pnlCapturedMarkets.has(key)) {
                    ...
                    this.calculateCurrentPnLWithLivePrices(marketToRemove).then(currentPnL => {
                        ...
                        // Log to watcher PnL tracker for TXT report
                        watcherPnLTracker.logMarketPnL(
                            fullMarketName,
                            marketToRemove.conditionId || '',
                            marketToRemove.marketKey,
                            marketToRemove.sharesUp,
                            marketToRemove.sharesDown,
                            marketToRemove.totalCostUp,
                            marketToRemove.totalCostDown,
                            marketToRemove.currentPriceUp || 0,
                            marketToRemove.currentPriceDown || 0,
                            marketToRemove.tradesUp,
                            marketToRemove.tradesDown
                        );
                    }).catch(() => {
            ...
            // Clear the key so next market window can be captured
            this.pnlCapturedMarkets.delete(key);
        }
    }
        ...
        for (const [key, value] of this.markets.entries()) {
            ...
            if (isClosed || isTimeWindowPassed || isStale) {
                ...
                if (value.investedUp > 0 || value.investedDown > 0) {
                    ...
                    // Capture PnL before deletion with live prices
                    if (!this.pnlCapturedMarkets.has(key)) {
                        ...
                        this.calculateCurrentPnLWithLivePrices(marketToCapture).then(currentPnL => {
                            ...
                            watcherPnLTracker.logMarketPnL(
                                fullMarketName,
                                marketToCapture.conditionId || '',
                                marketToCapture.marketKey,
                                marketToCapture.sharesUp,
                                marketToCapture.sharesDown,
                                marketToCapture.totalCostUp,
                                marketToCapture.totalCostDown,
                                marketToCapture.currentPriceUp || 0,
                                marketToCapture.currentPriceDown || 0,
                                marketToCapture.tradesUp,
                                marketToCapture.tradesDown
                            );
                        }).catch(() => {
                            // Fallback to sync version
        ...
        for (const result of results) {
            ...
            if (is15Min) {
                ...
                if (existingMarket && existingMarket.marketSlug !== slug) {
                    ...
                    // Clear the key so next market window can be captured
                    this.pnlCapturedMarkets.delete(marketKey);
                }
            }
